...
function Next({ onClick, arrowClass }) {
  return (
    <RightIcon
      ...
      className={cx(
        arrowClass,
        "absolute right-0 top-1/2 z-10 h-[50px] w-[50px] -translate-y-1/2 cursor-pointer md:right-4 lg:block xl:right-4"
      )}
    />
  );
}
function Prev({ onClick, arrowClass }) {
  return (
    <RightIcon
      ...
      className={cx(
        arrowClass,
        "absolute left-0 top-1/2 z-10 h-[50px] w-[50px] -translate-y-1/2 rotate-180 cursor-pointer md:left-4 lg:block xl:left-4"
      )}
    />
  );
}
